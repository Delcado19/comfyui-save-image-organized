...
- The local live ComfyUI custom-node install at `H:\ComfyUI-Easy-Install\ComfyUI\custom_nodes\comfyui-save-image-organized` is synced to `main` commit `53b60b7`; installed `nodes.py` imports successfully and exposes `SaveImageClean` as `Save Image Organized`.
